...
class SaliencyMetrics:
    ...
    def auc_shuff(
        self, s_map, gt, other_map, splits=100, stepsize=0.1, gt_threshold=255 / 2
    ):
        gt = np.where(gt < gt_threshold, 0, 1)
    ...
    def calculate_metrics(self, s_map_list, gt_list):
        ...
        for idx in range(len(gt_list)):
            auc_b.append(
                self.auc_borji(self.normalize_map(s_map_list[idx]), gt_list[idx])
            )
            auc_s.append(
                self.auc_shuff(
                    self.normalize_map(s_map_list[idx]), gt_list[idx], gt_list[idx]
                )
            )
