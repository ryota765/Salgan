...
def parse_args():
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        help="path to directrory of .npy file for input data",
        default="data",
    )
    parser.add_argument("--num_epoch", type=int, help="number of epochs", default=121)
    parser.add_argument("--batch_size", type=int, help="size of batch", default=32)
    parser.add_argument(
        "--l2_norm", type=float, help="parameter for l2 norm", default=0.0001
    )
    parser.add_argument(
        "--loss_alpha",
        type=float,
        help="weight alpha for loss calculation",
        default=0.005,
    )
    parser.add_argument(
        "--image_size",
        type=list,
        help="size of input image [width,height]",
        default=[256, 192],
    )
    parser.add_argument(
        "--learning_rate",
    ...
    parser.add_argument(
        "--model_save_ratio", type=int, help="ratio for model saving", default=5
    )
    parser.add_argument(
        "--load_model_path",
        type=str,
        help="Model path for using pre-trained model",
        default="model/weights_bce4_15.hdf5",
    )
...
def load_data(model_name, data_path):
    ...
    X_train /= 255
    Y_train = (
        Y_train.reshape(Y_train.shape[0], Y_train.shape[1], Y_train.shape[2], 1) / 255
    )
...
def train_bce(args):
    ...
    model_generator = model_builder.generator(
        img_width=img_width, img_height=img_height, l2_norm=l2_norm
    )

    model_generator.compile(
        loss=model.LossFunction().binary_crossentropy_forth,
    ...
    cp_cb = keras.callbacks.ModelCheckpoint(
        filepath=model_save_path, monitor="val_loss", verbose=1, period=model_save_ratio
    )
...
def train_salgan(args):
    ...
    model_discriminator = model_builder.discriminator(
        img_width=img_width, img_height=img_height, l2_norm=l2_norm
    )
    ...
    model_combine = model_builder.build_combine(
        model_generator, model_discriminator, img_width=img_width, img_height=img_height
    )

    model_discriminator.trainable = True
    model_discriminator.compile(
        optimizer=optimizers.Adagrad(lr=learning_rate), loss="binary_crossentropy"
    )
    ...
    for epoch in range(1, num_epoch + 1):
        ...
        for index in range(int(X_train.shape[0] / batch_size)):
            ...
            generated_salmap = model_generator.predict(
                x=image_batch, batch_size=batch_size
            )

            d_loss_real = model_discriminator.train_on_batch(
                np.concatenate([image_batch, salmap_batch], 3), output_true_batch
            )
            d_loss_fake = model_discriminator.train_on_batch(
                np.concatenate([image_batch, generated_salmap], 3), output_false_batch
            )
            ...
            c_loss = model_combine.train_on_batch(
                image_batch, [salmap_batch, output_true_batch]
            )
